...
{
	Log("OnRead ec:" + std::to_string(ec.value()));
...
void TalkToClient::OnWrite(const boost::system::error_code &ec, size_t bytes)
{
	...
	if (ec)
	{
		Log("OnWrite ec:" + std::to_string(ec.value()), std::cerr);
	}
...
void TalkToClient::OnLogin(std::string request)
{
	...
	channel_.lock()->OnLoggedIn(shared_from_this());

	Log("OnLogin");

	flatbuffers::FlatBufferBuilder builder;
	auto infoChannel = Protocols::CreateInfoChannel(builder, 0);
...
void TalkToClient::OnPing(std::string request)
{
	...
	flatbuffers::FlatBufferBuilder builder;
	auto infoChannel = Protocols::CreateInfoChannel(builder, channel_.lock()->id());
